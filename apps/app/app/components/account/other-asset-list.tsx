...
export default function OtherAssetList({ assets }: Props) {
    ...
    <>
      <table className="relative min-w-full table-fixed divide-y divide-stone-200 dark:divide-stone-600">
        ...
        <tbody className="divide-y divide-stone-200 bg-white dark:divide-stone-700 dark:bg-stone-800">
          {assets.map((p) => (
            <tr key={p.assetId} className="hover:bg-stone-100 dark:hover:bg-stone-700">
              <td
              ...
              <td className="p-4 text-right">{p.currency}</td>
            </tr>
          ))}
        </tbody>
      </table>
